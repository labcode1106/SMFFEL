...
def train(dataPath, modelSavePath):
    ...
    # random_state=3: 0.20968, random_state=7: 0.50645
    # random_state=0: 0.12595
    if dataPath == '../../data/ingotRate.csv':
    ...
    k_range = range(2, 31)
    regressor = DecisionTreeRegressor(max_depth=1, random_state=0)  # knn模型，这里一个超参数可以做预测，当多个超参数时需要使用另一种方法GridSearchCV
    ...
    plt.title('decisionTreeRegression', fontsize='large', fontweight='bold')
    plt.legend()
    plt.show()
    ...
    plt.title('Decision tree regression', fontsize='large', fontweight='bold')
    plt.grid(linewidth=0.3, linestyle='--')
    plt.xlabel('Indexes of samples')
    if dataPath == '../../data/ingotRate.csv':
        plt.ylabel('Ingot rate')
        saveName='ingotFittingDecisionTreeRegression.png'
    elif dataPath == '../../data/yieldRate.csv':
        plt.ylabel('Yield rate')
        saveName='yieldFittingDecisionTreeRegression.png'
    plt.legend()
    # plt.show()
...
def predict(predictionObject, data):
    ...
    if predictionObject == 'ingotRate':
        regressor = joblib.load('./model/decisionTreeRegressor/ingotRatePrediction.pkl')
    ...
    print('decisionTreeRegression result:\n', result[0])
    return result[0]
