...
def train(dataPath, modelSavePath):
    ...
    k_range = [0.1, 0.5, 1, 3, 5, 7, 10, 20, 100]
    regressor = Ridge(alpha=0.01)  # knn模型，这里一个超参数可以做预测，当多个超参数时需要使用另一种方法GridSearchCV
    ...
    # plt.plot([0.01] + [x for x in k_range], cv_scores)
    # plt.xlabel('alpha')
    ...
    plt.title('RidgeRegression', fontsize='large', fontweight='bold')
    plt.legend()
    plt.show()
    ...
    plt.title('Ridge regression', fontsize='large', fontweight = 'bold')
    plt.grid(linewidth=0.3, linestyle='--')
    plt.xlabel('Indexes of samples')
    if dataPath == '../../data/ingotRate.csv':
        plt.ylabel('Ingot rate')
        saveName='ingotFittingRidgeRegression.png'
    elif dataPath == '../../data/yieldRate.csv':
        plt.ylabel('Yield rate')
        saveName='yieldFittingRidgeRegression.png'
    plt.legend()
    # plt.show()
...
def predict(predictionObject, data):
    ...
    if predictionObject == 'ingotRate':
        regressor = joblib.load('./model/ridgeRegression/ingotRatePrediction.pkl')
        ...
        regressor = joblib.load('./model/ridgeRegression/yieldRatePrediction.pkl')
        testData = pd.DataFrame(data,
